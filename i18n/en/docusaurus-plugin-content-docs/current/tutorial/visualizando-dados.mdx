...
The dataset `agriculture_amazonas` provides crop production data from
...
With the data from `gdp_amazonas`, we can perform analyses regarding the
patterns observed in the distribution of data over the observed interval
(2010 to 2021).
...
# Selecting only year and taxes and plotting
gdp_amazonas %>%
...
Utilizing the `prf_amazonas` database, we can perform analyses of
accidents recorded by the Federal Highway Police (PRF) in the state of
...
# Counting accidents grouped by weather condition and plotting
prf_amazonas %>%
  count(weather) %>%
  mutate(
...
![](visualizando-dados.markdown_strict_files/figure-markdown_strict/unnamed-chunk-12-1.png)
