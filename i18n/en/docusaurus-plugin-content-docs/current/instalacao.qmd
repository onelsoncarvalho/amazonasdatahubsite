---
title: "Installation"
format: docusaurus-md
sidebar_position: 1
---

# Installation

Let's get started and install the `amazonasdatahub`.

## Start

### Requeriments

To install `amazonasdatahub`, you need to have the following tools installed on your computer or development environment:

- R version 4.41.1 (2024-06-14) -- "Race for Your Life";
- `devtools` package from R.

## Installing devtools


If you do not have `devtools` installed, you can install it using R terminal:

```{r}
#| eval: false
install.packages("devtools")
```


## Installing amazonasdatahub

After installing `devtools`, you can install the `amazonasdatahub` package through the `install_github()` method in the R terminal:

```{r}
#| title: instalação
#| eval: false
library(devtools)
install_github("Nelson-DevStack/amazonasdatahub")
```
You can also install using the `ìnstall_github()` method from `remotes` package:

```{r}
remotes::install_github("onelsoncarvalho/amazonasdatahub")
```

## Using in R

Once the `amazonasdatahub` is installed, use `library()` or `require()` to load the package.

```{r}
#| eval: false
library(amazonasdatahub)
```
